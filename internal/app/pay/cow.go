...
func (cow *COWResponse) isSucc() bool {
	return cow.Code == 0
}

type COWBalanceResponse struct {
	Status  bool           `json:"status"`
	Balance COWBalanceData `json:"balance"`
	Message string         `json:"message"`
}

func (cow *COWBalanceResponse) isSucc() bool {
	return cow.Status
}

type COWBalanceData struct {
...
func (p *COWPay) QueryBalance(params *PaymentParameters) (*BalanceResp, error) {
	...
	logger.ZInfo("COW QueryBalance", zap.String("url", params.PlatformApiUrl), zap.Any("req", data), zap.Any("result", result), zap.Error(err))
	if err != nil {
		return nil, err
	}

	var resp COWBalanceResponse
...
func (p *COWPay) RequestPaymentURL(params *PaymentParameters) (*PaymentResp, error) {
	data := map[string]string{
		"merchant_code": params.MerNo,
	...
	logger.ZInfo("COW RequestPaymentURL", zap.String("url", params.PlatformApiUrl), zap.Any("req", data), zap.Any("result", result), zap.Error(err))

	if err != nil {
		return nil, err
	}

	var resp COWResponse
...
func (p *COWPay) sign(data map[string]string, privateKey string) string {
	// 对map的键进行排序
...
func (p *COWBack) sign(mkey string) string {
...
func (p COWBack) VerifySignature(appKey string) bool {
	return p.sign(appKey) == strings.ToUpper(p.Sign)
}

// 只有支付成功才会通知
func (p COWBack) IsTransactionSucc() bool {
	return true
}

func (p COWBack) GetTransferOrder() *TransferOrder {
...
func (p COWBack) GetSuccResp() string {
	return "SUCCESS"
}
...
func (cow *COWWithdrawResponse) isSucc() bool {
	return cow.Status
}

type COWWithdraw struct {
}

func (p *COWWithdraw) RequestWithdraw(params *WithdrawParameters) (*WithdrawResp, error) {
	data := map[string]string{
	...
	logger.ZInfo("COW RequestWithdraw", zap.String("url", params.PlatformApiUrl), zap.Any("data", data), zap.Any("result", result), zap.Error(err))

	if err != nil {
		return nil, err
	}

	var resp COWWithdrawResponse
	structure.MapToStruct(result, &resp)
	if resp.isSucc() {
		return &WithdrawResp{
...
func (p *COWWithdraw) sign(data map[string]string, privateKey string) string {
	// 对map的键进行排序
	keys := make([]string, 0, len(data))
	for k := range data {
		keys = append(keys, k)
...
func (p *WithdrawCOWBack) sign(mkey string) string {
...
func (p WithdrawCOWBack) VerifySignature(appKey string) bool {
	return p.sign(appKey) == strings.ToUpper(p.Sign)
}

func (p WithdrawCOWBack) IsTransactionSucc() bool {
	return p.RespCode == "S" //S 代付成功，F 代付失败，P 代付中
}

func (p WithdrawCOWBack) GetTransferOrder() *TransferOrder {
	amount, _ := strconv.ParseFloat(p.OrderAmount, 64) // 将字符串转换为float64类型
	return &TransferOrder{
		MerOrderNo:  p.OrderNo,
		OrderNo:     p.OrderNo,
...
func (p WithdrawCOWBack) GetSuccResp() string {
	return "SUCCESS"
}
